...
describe('Testa renderização do layout', () => {
  it('Verifica que os números NÃO são exibidos antes do pagamento', () => {
    const wrapper = mount(PsrShow, {
      props: getValidData(),
    });

    // Não deve conter botão de alternar números antes do pagamento
    expect(wrapper.find('[data-testid="button-toggle-numbers"]').exists()).toBeFalsy();
    // Deve conter o aviso de que números serão gerados após pagamento
    expect(wrapper.text()).toContain('Números gerados após o pagamento');
  });
